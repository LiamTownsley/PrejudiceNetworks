...
module.exports = (bot, message) => {
    ...
    if (!message.content.startsWith(prefix)) {
        return bot.channels.cache.get('722811357715759114').send({
            embed: {
    ...
    bot.channels.cache.get('722813963401953302').send({
        embed: {
    ...
    else if(bot.permissionGroups[message.member.roles.highest.id] > message.member.permissionLevel) {
        return message.reply(`You have insufficient permission to run \`${command.name}\`.`);
    }
    ...
    if (!command.essential &&
        ...
        !message.member.permissionLevel > 3) {
