...
module.exports = {
	...
	async execute(message, args, bot) {
		...
		let ticketChannel = message.guild.channels.create(`ticket-${ticketNumber}`, {
                type: 'text',
			...
			.then(channel => {
				(async () => {
					...
					for (question of questions) {
						let answer = await askQuestion(question, message, channel)
						if (!answer) {
					...
					.then(msg => {
						...
						.then(channel => {
							channel.send("<@&723921845216215050>")
							msg.pin();
						})
